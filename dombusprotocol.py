...
#                                                        ha['device_class'] = 'motion' if portName == 'Touch' else 'door'
    ...
    def moduleUpdate(self):
        ...
        if self.frameAddr not in Modules:
            Modules[self.frameAddr] = [ int(time.time()), int(time.time()*1000), int(time.time())+3-DB.PERIODIC_STATUS_INTERVAL, 0, 'Unknown', 'Unknown']
        else:
...
class DomBusManager:
    ...
    async def _mqttSubscribe(self):
        ...
        await mqtt['client'].subscribe("#", options={"no_local": True})  # Subscribe to all topics
        log(DB.LOG_DEBUG, f"[MQTT] Subscribed to all topics ('#')")
