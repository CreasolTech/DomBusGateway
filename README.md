...
* _showbus BUS_ : list modules attached to the specified bus (it's possible to connect 20-30 modules to the same bus, but for safety reasons it can be good to differentiate bus by floors or by area to manage very large buildings). For example _showbus 2_ to list modules attached to bus #2 ![showbus command](https://images.creasol.it/showbus.webp)
...
* _setport PORT CONFIGURATION_ : change the configuration of the specified port. For example _setport 1 IN_COUNTER,DIVIDER=2000_ to configure port1 in counter mode, type energy meter with pulsed output, 2000 pulses / kWh. ![setport command example](https://images.creasol.it/setport.webp)
